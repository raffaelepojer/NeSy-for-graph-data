...
class MLP(nn.Module):
    def __init__(self, nfeat, nlayers, nhidden, nclass, dropout, use_res, primula=False):
...
"""
Modifided version of GGCN 
Here all the data precomputation is made on the model, for easy support in Primula.
The first train should be slower for the precomputation in the model.
"""
class GGCN_raf(nn.Module):
    def __init__(self, nfeat, nlayers, nhidden, nclass, dropout, decay_rate, exponent, use_degree=True, use_sign=True, use_decay=True, use_sparse=False, scale_init=0.5, deg_intercept_init=0.5, use_bn=False, use_ln=False, generated=False, pre_features=False, primula=False):
...
class myGCNconv(nn.Module):
    """
    changed from the original, since primula construct the edge_index with a (2xnum_edges) shape
    Simple GCN layer, similar to https://arxiv.org/abs/1609.02907
    """
    ...
    def forward(self, input, edge_index, use_iden=False):
        n = input.size(0)
        values = torch.ones(edge_index.size(1))
        adj = torch.sparse_coo_tensor(
            edge_index, values, (n, n)
        )
...
#         for _ in range(nlayers-1):
#             self.convs.append(GCNConv(nhid, nhid, improved=True, normalize=True))
#         self.lin = torch.nn.Linear(nhid, nclass)
#         self.primula = primula
