...
for set in settings:
    ...
    for r in range(10):
        ...
        opt['dataset'] = f'../data/ising/{N}_{J}_{Jb}_{temp}_noisy'
        opt['hidden_dim'] = 16
        ...
        feature.to_one_hot(binary=False)
        ...
        def train_p(epoches):
            ...
            for epoch in range(epoches):
                ...
                results += [(accuracy_dev, accuracy_test, loss)]
            return results
        ...
        print(acc_test_p * 100)
        accs_p.append(acc_test_p * 100)
        ...
        epochs = range(len(p_results))
        p_accuracy = [acc[1] for acc in p_results]
        p_test = [acc[0] for acc in p_results]

        plot_p.append((p_accuracy, p_test))
        # plt.figure()
        # plt.plot(epochs, p_accuracy, label='Dev Accuracy')
        # plt.plot(epochs, p_test, label='Test Accuracy')
        # plt.show()
    
    figure, axis = plt.subplots(5, 2)
    for a, p in enumerate(plot_p):
        axis[a%5, a%2].plot(range(len(p[0])), p[0], label='p acc')
        axis[a%5, a%2].plot(range(len(p[1])), p[1], label='p loss')

    plt.show()

    print(f'{set} mean:{np.mean(accs_p)} std:{np.std(accs_p)}')
