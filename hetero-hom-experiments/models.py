...
# Basic GCN model for testing
class GraphGCN(torch.nn.Module):
    def __init__(self, dim_input, num_layers, dim=16, num_classes=1, dropout=0.5, primula=False):
        super(GraphGCN, self).__init__()
        self.num_classes = num_classes
        self.dropout = dropout
        self.convs = torch.nn.ModuleList()
        self.convs.append(GCNConv(dim_input, dim, improved=False))
        for _ in range(num_layers-1):
            self.convs.append(GCNConv(dim, dim, improved=False))
