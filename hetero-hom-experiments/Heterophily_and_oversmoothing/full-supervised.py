...
def train(datastr,splitstr):
    ...
    if args.model=="GCN":
        ...
        model = GGCN_prim(nfeat=features.shape[1], nlayers=args.layer, nhidden=args.hidden, nclass=num_labels, dropout=args.dropout, decay_rate=args.decay_rate, exponent=args.exponent, use_degree=use_degree, use_sign=use_sign, use_decay=use_decay, use_sparse=args.use_sparse, scale_init=args.scale_init, deg_intercept_init=args.deg_intercept_init, use_bn=use_bn, use_ln=use_ln, generated=True, pre_features=True, primula=False).to(device)
        if not args.use_sparse:
            adj = adj.to_dense()
    elif args.model=="MLP":
